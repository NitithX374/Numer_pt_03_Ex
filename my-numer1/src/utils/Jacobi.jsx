    ...
    const calculateJacobi = () => {
        try {
            ...
            for (let iteration = 0; iteration < maxIterations; iteration++) {
                ...
                stepLog.push(
                    `Iteration ${iteration + 1}: [${newGuess.map((val) => val.toFixed(6)).join(", ")}]`
                );

                // Check convergence
                hasConverged = newGuess.every((val, idx) =>
                    Math.abs(val - currentGuess[idx]) < tolerance
                );
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4">
            <div className="w-full max-w-6xl bg-white rounded-lg shadow-lg p-6">
                <h2 className="text-3xl font-bold text-center mb-6">Jacobi Iteration Calculator</h2>
...
export default JacobiCalculator;
